...
## v0.9 — Antigravity 監督審查

### 實作

- 擴充 Antigravity 角色，使其在實質工作檢查點擔任 Codex 的獨立監督 reviewer。
- 加入重大問題的證據核對、修正、聚焦複核與未解風險紀錄流程。
- 保留既有每個請求最多兩次呼叫及輔助 worker 唯讀限制。
- 第一次 Antigravity 監督指出觸發邊界、重大問題定義、brief 欄位與通過狀態不夠明確；Codex 接受並加入實質工作定義、三段嚴重度、三種狀態及聚焦驗證規則。
- 未採用「Blocker 時允許第三次呼叫」建議；第二次仍有 Blocker 時改為停止交付並交由使用者決定，以維持明確成本上限。
- 新增 GitHub 權限邊界：已連線帳號的唯讀操作不再重複詢問；只有上傳本機／新產生內容或改變遠端狀態時需要明確授權，當次要求已包含動作時不重複確認。
- 保留執行環境強制 sandbox 核准的例外說明，避免把平台限制誤認為協作層再次詢問。
- 第一次 Antigravity 權限審查判定 `REJECTED`：原規則可能被解讀為遍歷帳號下所有私人 repo，且一般 push 未限定目標。Codex 接受並限制為當前／已點名 repo，補上 feature branch／PR 預設、protected branch 與 force-push 明確授權、Actions 寫入分類及安全下載規則。
- 第二次也是最後一次聚焦審查確認四項權限升級風險均已解決，回傳 `STATUS: RESOLVED`，沒有剩餘 Blocker／Warning；狀態字串未完全遵循預定三選一格式，Codex 如實保留此格式偏差且不啟動第三次呼叫。

### 設計取捨

| 問題 | 決定 | 原因 |
|---|---|---|
| 是否讓 Antigravity 全程監看每一步 | 只在關鍵檢查點啟動 | 控制延遲、額度與上下文成本 |
| Antigravity 與 Codex 意見不同時由誰決定 | Codex 核對證據並保留分歧，重大未解事項交由使用者 | 避免假共識並維持使用者最終權限 |
| 是否允許 Antigravity 修檔 | 不允許 | 維持單一修改者與可追責的驗證流程 |
| GitHub 何時詢問權限 | 唯讀持續授權；遠端寫入才詢問 | 減少已連線帳號的重複確認，同時保護發布與遠端狀態 |
